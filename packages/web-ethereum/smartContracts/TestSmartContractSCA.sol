...
//This is the smart contract that is deployed
...
contract TestSmartContractSCA{
    ...
    function getAcknowledge(string memory _messageRequest, bytes32 _messageHash, uint8 _v, bytes32 _r, bytes32 _s) external {
        require(protocolActive == true, "The protocol must be active");
        if (keccak256(abi.encodePacked(_messageRequest)) == keccak256(abi.encodePacked("messageAcknowledge"))){
            emit AcknowledgeMessageCommittedA(msg.sender, "AcknowledgeMessageCommittedA: The signer of this transaction requests transmitting the attached message to the recepient",_messageRequest, block.timestamp);
        } else {
